...
# Program: MACOS 14.3.1, RStudio R version 4.2.2
...
pacman::p_load(readr, tidyverse, magrittr,  dplyr, devtools,
               reshape2,  kableExtra, ggplot2, gtsummary, arsenal, ggpubr, cowplot)
...
Samples_VERDI_Coprimary <- Samples_VERDI %>% select(ref_id, sample_type, collection_date, result) %>%
  arrange(ref_id, collection_date, desc(result)) %>% distinct(ref_id, sample_type, .keep_all=T) %>%
...
AgeIndex <- AllParticipants %>% filter(Index=="index") %>% select(household_id, Age_group) %>% distinct() %>%  rename(Age_group_index = Age_group) 
...
VERDIpp <- merge(VERDIpp, hhm_refids, by="ref_id", all=T) %>% filter(IncludedAntibodies=="Yes") 
...
Agectvalue= Samples_VERDI %>% filter(!is.na(`MERS-NP_median`)& !is.na(sample_type)) %>% 
  ...
  facet_grid(.~sample_type)  + scale_colour_brewer(palette = "Set1") +theme_bw()
Severity = Samples_VERDI %>% filter(!is.na(`MERS-NP_median`)& !is.na(sample_type)) %>% ggplot(aes(daytopeak, result_ct, colour=Diseaseseverity)) +geom_vline(xintercept=0) +  geom_point(alpha=0.4) + geom_smooth() + 
  scale_x_continuous(limits = c(-15, 20.1), expand = c(0,0))  + 
...
Samples_VERDI %>% filter(!is.na(`MERS-NP_median`) & sample_type=="nts") %>% 
  dplyr::select(-contains(c("B_", "MERS", "Influenza", "SARS-ecto_start", "SARS-S1_start", "SARS-CoV-2 WT RBD_start", "SARS-NP_start"))) %>% 
  pivot_longer(contains("_median"), names_to = "Virus", values_to = "value") %>% 
  ...
  scale_x_continuous(limits = c(-15, 20.1), expand = c(0,0))  + 
  scale_y_reverse(limits = c(40.2,10), expand = c(0,0))  + scale_colour_brewer(palette = "Set1") + labs(colour="Antibody titer", x= "Day to peak", y="Ct value") +
  facet_grid(target~Virus)  + scale_colour_brewer(palette = "Set1") +theme_bw() +theme(legend.position = "bottom")

#### Supplementary Fig. 1 Correlation of cumulative high antibody levels for different coronaviruses measured at baseline. #### 
library(corrplot); library(PerformanceAnalytics)
sub <- Viralkinetics %>% dplyr::select(contains("Cum")) 
colnames(sub) <- gsub("^Cum_", "", colnames(sub))
sub = sub %>% select(-SARS1, -MERS, -Influenza) 
